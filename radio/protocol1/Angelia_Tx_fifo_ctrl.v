...
module Angelia_Tx_fifo_ctrl(IF_reset, IF_clk, Tx_fifo_wdata, Tx_fifo_wreq, Tx_fifo_full, Tx_fifo_used,
                    ...
                    ADC2_OVERLOAD, Penny_serialno, Merc_serialno, Orion_version, Penny_ALC, AIN1, AIN2, AIN3, 
                    AIN4, AIN6, IO4, IO5, IO6, IO8, VNA_start, VNA, pk_detect_reset, pk_detect_ack);
...
input  reg 		pk_detect_ack;
output reg		pk_detect_reset;			// signal to begin new peak detect interval
...
begin 
  case (AD_state)
    ...
    AD_SEND_SYNC2:
    begin  
      Tx_fifo_wdata   = {8'h7F, tx_addr, clean_dot, clean_dash, clean_PTT_in}; //third sync and C0
      Tx_fifo_wreq    = 1'b1;
		if (tx_addr == 5'd2) pk_detect_reset <= 1'b1;			// signal pk detect interval to be reset in Orion_ADC.v
  ...
  endcase
  if (pk_detect_ack == 1'b1) pk_detect_reset <= 1'b0;	// reset was received, remove reset signal
end
